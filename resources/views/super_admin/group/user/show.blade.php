<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 p-3">
        <div class="mt-1">
            ...
            <div class="text-xl">
                <span class="text-gray-500">الحساب: </span>
                {{ __($user->user_type) }}
            </div>

            <div class="text-xl">
                <span class="text-gray-500">الوفد أو المجموعة: </span>
                {{ $user->group->title }}
            </div>
        ...
        تحديث كلمة المرور (ستكون نفس رقم الهاتف)
            <form method="post" action="{{ route('super_admin.group.user.password.update',$user->id) }}">
                @csrf
